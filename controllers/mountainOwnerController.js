  ...
  upload(req, res, async function (err) {
    try {
      ...
      const newRequest = new MountainRequest({
        ...
        pointsOfInterest: parseJSONFile(
          req.files.pointsOfInterestFile
            ? req.files.pointsOfInterestFile[0]
            : null
        ),
...
exports.getRequestStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      message: `Your request is currently ${request.status}.`,
    });
  } catch (error) {
...
exports.updateMountainRequest = async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: "Mountain request updated successfully.",
