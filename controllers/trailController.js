...
exports.getTrailsByMountain = async (req, res) => {
  try {
    ...
    if (!trails.length) {
      return res
        .status(404)
        .json({ message: "No trails found for this mountain." });
    }
...
exports.getTrailsByLift = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving trails by lift.", error });
  }
};
...
exports.getUniqueDifficulties = async (req, res) => {
  try {
    ...
    const uniqueDifficulties = [
      ...new Set(trails.map((trail) => trail.difficulty)),
    ];
    ...
    res
      .status(500)
      .json({ message: "Error retrieving unique difficulties.", error });
  }
};
...
exports.getRunNameByRunID = async (req, res) => {
  try {
    ...
    if (!runID || !mountainID) {
      return res
        .status(400)
        .json({ message: "runID and mountainID are required." });
    }
...
exports.getRunIDByRunName = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving runID by runName.", error });
  }
};

exports.getTrailDifficulty = async (req, res) => {
  try {
    console.log("Received request:", req.method, req.originalUrl);
    console.log("Request body:", req.body);

    const { trailNames } = req.body;
    if (!trailNames || !trailNames.length) {
      return res.status(400).json({ message: "Trail names are required." });
    }

    const trails = await Trail.find({ runName: { $in: trailNames } });
    const difficulties = trails.reduce((acc, trail) => {
      acc[trail.runName] = trail.difficulty;
      return acc;
    }, {});
